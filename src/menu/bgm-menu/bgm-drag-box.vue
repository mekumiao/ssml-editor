...
onMounted(() => {
  emitter.on(EMITTER_EVENT.BGM_MENU_CLICK, handleMenuClick)
})

onUnmounted(() => {
  emitter.off(EMITTER_EVENT.BGM_MENU_CLICK, handleMenuClick)
})
...
async function handleMenuClick(pot: Position) {
  ...
  visible.value = true
  dataList.value ??= await fetchBgm({ search: '', menuKey: 'first', scene: '', style: '' })
}

function handleSubmit(value: LabelValue) {
  visible.value = false
  emitter.emit(EMITTER_EVENT.BGM_DRAG_BOX_SUBMIT, value)
}
</script>
